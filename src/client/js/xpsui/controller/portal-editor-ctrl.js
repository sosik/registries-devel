...
(function(angular) {
	...
	//FIXME correct errorhandling
	notificationFactory.error({translationCode: 'registry.unsuccesfully.saved', time: 3000, err: err});
      });
	} else {
		$http({
			method: 'POST',
	...
	$scope.showBlockSelector = function showBlockSelector() {
		...
		angular.element($document[0].body).addClass('x-dropdown-open');
	};

	$scope.hideBlockSelector = function hideBlockSelector() {
		$scope.blockSelectorShown = false;
		angular.element($document[0].body).removeClass('x-dropdown-open');
	};
	...
	/**
	 * Returns relative url to template defined in article (model)
	 *
	 * @return relative url for template of null if there is no template defined
	 */
